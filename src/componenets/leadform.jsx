                    ...
                    <div className="grid md:grid-cols-2 gap-12 items-center">
                        ...
                        <div className="space-y-6">
                            <h2 className="text-3xl md:text-4xl font-black text-slate-900 dark:text-white leading-tight">
                                Ton ROAS est peut-être <span className="text-primary">faux.</span>
                            </h2>
                            <p className="text-lg text-slate-500 font-medium">
                                Le vrai KPI : bénéfice livré, pas likes ni clics. Réservez une démo pour voir comment RiseManager révèle vos vrais chiffres.
                            </p>
                            ...
                            <div className="space-y-4 pt-4">
                                <div className="flex items-center gap-3 p-4 rounded-2xl bg-white dark:bg-slate-950 border border-slate-100 dark:border-white/5">
                                    ...
                                    <span className="text-sm font-bold text-slate-700 dark:text-slate-300">Tu scales des campagnes qui te ruinent.</span>
                                </div>
                                <div className="flex items-center gap-3 p-4 rounded-2xl bg-white dark:bg-slate-950 border border-slate-100 dark:border-white/5">
                                    <AlertCircle className="w-5 h-5 text-amber-500" />
                                    <span className="text-sm font-bold text-slate-700 dark:text-slate-300">Tu coupes des campagnes rentables.</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white dark:bg-slate-950 rounded-3xl p-6 md:p-8 border border-slate-200 dark:border-white/10 shadow-lg relative">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4 relative z-10">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div className="space-y-1.5">
                                        <label className="text-xs font-bold text-slate-500 uppercase">Budget mensuel</label>
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-xs font-bold text-slate-500 uppercase">Plateforme Principale</label>
                                    <div className="grid grid-cols-3 gap-2">
                                        {['Meta', 'TikTok', 'Les deux'].map(plat => (
                                            <button 
                                                type="button" 
                                                key={plat}
                                                onClick={() => setFormData({...formData, plateforme: plat})}
                                                className={`py-2 text-xs font-bold rounded-lg border transition-all ${
                                                    formData.plateforme === plat 
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-xs font-bold text-slate-500 uppercase">Problème principal</label>
                                    <select name="probleme" required className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm font-medium text-slate-900 dark:text-white focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all appearance-none" onChange={handleChange}>
                                        <option value="">Sélectionner</option>
                                        <option value="CPA haut">CPA trop haut</option>
                                        <option value="scaling">Problème de scaling</option>
                                        <option value="tracking">Tracking incertain</option>
                                        <option value="rentabilité">Rentabilité globale</option>
                                    </select>
                                </div>
